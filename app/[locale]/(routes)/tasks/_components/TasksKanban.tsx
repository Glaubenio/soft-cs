...
const ClientsKanban = () => {
  const { tasks, deleteTask, deleting, isLoading, refresh } = useContext(TasksContext);
  ...
  const onDragEnd = async ({ source, destination }: DropResult) => {
    ...
    refresh(updatedTasks, { revalidate: false });
                ...
                <Droppable
                  ...
                  {(provided) => (
                    <Collapsible
                      ...
                      key={section.id}
                    >
                      <div key={section.id} className="flex flex-row justify-between items-center w-full py-[16px] px-[14px] shadow-soft-cs rounded-[12px] text-[16px] bg-white font-[700] border-0 border-t-4 border-solid border-primary">
